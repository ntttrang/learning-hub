...
/**
 * Learning progress statistics. (Ported from learn-dp-800/src/lib/progress.ts;
 * the static curriculum imports became an explicit `SubjectContent` parameter,
 * so the engine works for any subject pack.)
 */
...
export interface DerivedStats {
  ...
  domainExam: Record<string, number>; // accuracy from exam attempts
...
export function computeStats(
  ...
  const completedLessons = content.lessons.filter((l) => lessons[l.id]?.status === 'completed').length;
  const totalLessons = content.lessons.length;

  const domainCompletion: Record<string, number> = {};
  for (const d of content.domains) {
    const domainLessons = content.lessons.filter((l) => l.domainId === d.id);
    const done = domainLessons.filter((l) => lessons[l.id]?.status === 'completed').length;
    domainCompletion[d.id] = domainLessons.length ? done / domainLessons.length : 0;
  }
  ...
  return {
    ...
    labsDone: completedLabs.filter((id) => content.labs.some((l) => l.id === id)).length,
    domainCompletion,
    domainExam,
    weakDomains,
  };
