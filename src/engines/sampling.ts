 ...
 * (PRNG + shuffle ported verbatim from learn-gh-200/src/utils/sample.ts; the
 * sampling entry points now read the unified `Exam.selection.sampled` shape.)
 *
...
/** Narrow a unified exam to its sampling plan, or explain why it has none. */
function sampledPlan(exam: Exam): { id: string; domainPlan: Record<string, number>; seed: number } {
  const { selection } = exam;
  if (selection.kind !== 'sampled') {
    throw new Error(`${exam.id}: exam selection is "${selection.kind}", not "sampled"`);
  }
  return { id: exam.id, domainPlan: selection.domainPlan, seed: selection.seed };
}

/**
 * The question ids a sampled exam draws, in serving order.
 *
...
export function sampleIds(
  ...
  const { id, domainPlan, seed } = sampledPlan(exam);
  const rng = mulberry32(seed);
  const picked: string[] = [];
  for (const [domainId, count] of Object.entries(domainPlan)) {
    const pool = questions.filter(
      (question) => question.domainId === domainId && !excludeIds.has(question.id),
    );
    if (pool.length < count) {
      throw new Error(
        `${id}: domain ${domainId} needs ${count} questions but only ${pool.length} are available`,
      );
    }
    picked.push(...shuffleWith(rng, pool).slice(0, count).map((question) => question.id));
  }
  return shuffleWith(mulberry32(seed ^ 0x5bd1e995), picked);
}
